...
function teamLogoUrl(teamName, sportName) {
...
function tournamentBannerUrl(i) {
    return `https://picsum.photos/seed/tournament${i}/1200/400`;
}
function postImageUrl(sport, i) {
...
function makeScore(sport, completed, winnerA) {
    if (!completed) {
...
function cricketEvents(matchId, userId) {
    const over = (o, b) => ({
        match_id: matchId,
        event_type: 'ball',
        period: o,
...
// Handler
// ────────────────────────────────────────────────────────────────────────────
async function loadFullData(req, res) {
    ...
    try {
        ...
        for (const [name, id] of sportIdByName.entries())
            sportNameById.set(id, name);
        ...
        for (let i = 0; i < NUM_USERS; i++) {
            ...
            let cityName = 'Pune';
            if (pick >= 0.60 && pick < 0.75)
                cityName = 'Mumbai';
            else if (pick >= 0.75 && pick < 0.85)
                cityName = 'Bengaluru';
            else if (pick >= 0.85 && pick < 0.95)
                cityName = 'Delhi';
            else if (pick >= 0.95)
                cityName = rand(['Hyderabad', 'Chennai']);
            const cityId = cityByName.get(cityName) ?? puneId;
            // Role mix: 120 players, 15 umpires, 8 coaches, 7 businesses
            let accountType = 'Player';
            if (i >= 120 && i < 135)
                accountType = 'Umpire-Referee';
            else if (i >= 135 && i < 143)
                accountType = 'Trainer-Coach';
            else if (i >= 143)
                accountType = 'Business-Vendor';
            userRows.push({
        ...
        const sportPlayerIds = new Map();
        for (const s of SPORT_NAMES)
            sportPlayerIds.set(s, []);
        ...
        for (const sportName of SPORT_NAMES) {
            ...
            for (const teamName of TEAM_DATA[sportName]) {
                // Map team name → city
                let teamCity = puneId;
                if (teamName.startsWith('Mumbai'))
                    teamCity = cityByName.get('Mumbai') ?? null;
                else if (teamName.startsWith('Bengaluru'))
                    teamCity = cityByName.get('Bengaluru') ?? null;
                else if (teamName.startsWith('Delhi'))
                    teamCity = cityByName.get('Delhi') ?? null;
                teamRows.push({
                    sport_id: sid,
                    name: teamName,
        ...
        const { data: existingTeams } = await supabase_1.supabase
            .from('teams')
            .select('id, name, sport_id')
            .in('name', targetTeamNames);
        const existingTeamKeys = new Set((existingTeams ?? []).map((t) => `${t.sport_id}::${t.name}`));
        const newTeamRows = teamRows.filter((t) => !existingTeamKeys.has(`${t.sport_id}::${t.name}`));
        ...
        const existingTourneyKeys = new Set((existingTourneys ?? []).map((t) => `${t.sport_id}::${t.name}`));
        const newTourneyRows = tournamentRows.filter((t) => !existingTourneyKeys.has(`${t.sport_id}::${t.name}`));
        ...
        if (newTourneyRows.length > 0) {
            for (let i = 0; i < newTourneyRows.length; i += 20) {
                ...
                const { data, error } = await supabase_1.supabase
                    ...
                    .select('id, name, sport_id, status');
                if (error)
                    summary.errors.push(`tournaments[${i}]: ${error.message}`);
                if (data) {
                    summary.tournaments_created += data.length;
                    for (const t of data)
                        freshlyInsertedTourneys.push(t);
                }
            }
        }
        // Re-fetch all target tournaments (used later for general lookups)
        const { data: allTourneys } = await supabase_1.supabase
            .from('tournaments')
            .select('id, name, sport_id, status')
            .in('name', targetTournamentNames);
        ...
        const tourneyIdsNeedingMatches = new Set(freshlyInsertedTourneys.map((t) => t.id));
        ...
        if (existingTourneyIdsToCheck.length > 0) {
            ...
            for (const tid of existingTourneyIdsToCheck) {
                if (!tourneysWithMatches.has(tid))
                    tourneyIdsNeedingMatches.add(tid);
            }
        }
        const newTourneyIds = tourneyIdsNeedingMatches;
        const matchRows = [];
        const completedMatchBuckets = [];
        for (const t of allTourneys ?? []) {
            if (!newTourneyIds.has(t.id))
                continue;
        ...
        for (let i = 0; i < matchRows.length; i += 100) {
            ...
            if (error)
                summary.errors.push(`matches[${i}]: ${error.message}`);
            if (data) {
        ...
        if (cricketSportId) {
            ...
            for (let i = 0; i < matchRows.length && cricketCompletedIds.length < 3; i++) {
                ...
                if (row.sport_id === cricketSportId && row.status === 'completed') {
                    ...
                    if (insertedId)
                        cricketCompletedIds.push(insertedId);
                }
            }
            const eventRows = [];
            for (const mid of cricketCompletedIds)
                eventRows.push(...cricketEvents(mid, userId));
            if (eventRows.length > 0) {
                for (let i = 0; i < eventRows.length; i += 100) {
                    const chunk = eventRows.slice(i, i + 100);
                    const { data, error } = await supabase_1.supabase.from('match_events').insert(chunk).select('id');
                    if (error)
                        summary.errors.push(`match_events[${i}]: ${error.message}`);
        ...
        const { data: existingVenues } = await supabase_1.supabase
            .from('venues')
            .select('id, name')
            .in('name', venueNames);
        ...
        if (newVenues.length > 0) {
            const { data } = await supabase_1.supabase.from('venues').insert(newVenues).select('id');
        ...
        if (postRows.length > 0) {
            for (let i = 0; i < postRows.length; i += 30) {
                ...
                const { data } = await supabase_1.supabase.from('community_posts').insert(chunk).select('id');
        ...
        if (notifRows.length > 0) {
            const { data } = await supabase_1.supabase.from('notifications').insert(notifRows).select('id');
        ...
        if (giftRows.length > 0) {
            const { data } = await supabase_1.supabase.from('gift_transactions').insert(giftRows).select('id');
        ...
        for (const uid of outgoing)
            followRows.push({ follower_id: userId, following_id: uid });
        for (const uid of incoming)
            followRows.push({ follower_id: uid, following_id: userId });
        ...
        const { data: existingChallenges } = await supabase_1.supabase
            .from('challenges')
            .select('id, title')
        ...
        if (newChallenges.length > 0) {
            const { data } = await supabase_1.supabase.from('challenges').insert(newChallenges).select('id, title');
            summary.challenges_created = data?.length ?? 0;
        }
        const { data: allChallenges } = await supabase_1.supabase
            .from('challenges')
            .select('id, title')
            .in('title', challengeTitles);
        const challengeByTitle = new Map();
        for (const c of allChallenges ?? [])
            challengeByTitle.set(c.title, c.id);
        const userChallengeRows = [];
        ...
        const { data: coinData } = await supabase_1.supabase
            .from('coin_events')
        ...
        const { data: callerExisting } = await supabase_1.supabase
            .from('users')
            .select('referral_code')
            .eq('id', userId)
            .maybeSingle();
        if (!callerExisting?.referral_code)
            callerPatch.referral_code = 'SC-DIPAK1';
        await supabase_1.supabase.from('users').update(callerPatch).eq('id', userId);
        // Add caller to Pune Warriors cricket team
        if (cricketSportId) {
            const { data: puneWarriors } = await supabase_1.supabase
                .from('teams')
                .select('id')
        ...
        const { data: completedForCaller } = await supabase_1.supabase
            .from('matches')
            .select('id')
            .eq('status', 'completed')
        ...
        if (participantRows.length > 0) {
            const { data } = await supabase_1.supabase.from('match_participants').insert(participantRows).select('id');
...
exports.loadFullData = loadFullData;
