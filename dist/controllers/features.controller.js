...
exports.triggerWeeklyDigest = exports.triggerReEngagement = exports.checkRatingMilestone = exports.triggerSmartMatchNotifications = exports.getNearbyMatches = exports.getPlayerOfWeek = exports.getSeasonRecap = exports.publishScheduledPosts = exports.getTournamentAnalytics = exports.getTournamentOfficials = exports.removeTournamentOfficial = exports.addTournamentOfficial = exports.getTournamentTopPerformers = exports.getTournamentStandings = void 0;
...
async function getTournamentStandings(req, res) {
    try {
        ...
        const { data: sport } = await supabase_1.supabase.from('sports').select('slug').eq('id', tournament.sport_id).maybeSingle();
        ...
        const standings = Array.from(table.values()).sort((a, b) => b.points - a.points || b.nrr - a.nrr || b.won - a.won);
        ...
        if (tournament.format === 'groups_knockout') {
            ...
            for (const grp of groups.values()) {
                grp.slice(0, 2).forEach((r) => r.qualified = true);
            }
        }
...
// ────────────────────────────────────────────────────────────────────────────
async function getTournamentTopPerformers(req, res) {
    try {
        const { id } = req.params;
        const { data: tournament } = await supabase_1.supabase
            .from('tournaments')
        ...
        const { data: teams } = await supabase_1.supabase
            .from('teams')
            .select('id, name')
...
exports.getTournamentTopPerformers = getTournamentTopPerformers;
// ────────────────────────────────────────────────────────────────────────────
// TOURNAMENT OFFICIALS
// ────────────────────────────────────────────────────────────────────────────
async function addTournamentOfficial(req, res) {
    ...
    try {
        ...
        const { data: tournament } = await supabase_1.supabase.from('tournaments').select('created_by').eq('id', id).maybeSingle();
        if (!tournament)
            return res.status(404).json({ error: 'Tournament not found' });
        if (tournament.created_by !== userId)
            return res.status(403).json({ error: 'Only organiser can add officials' });
...
exports.addTournamentOfficial = addTournamentOfficial;
async function removeTournamentOfficial(req, res) {
    const userId = req.userId;
    if (!userId)
        return res.status(401).json({ error: 'Unauthorized' });
    try {
        ...
        if (!tournament)
            return res.status(404).json({ error: 'Tournament not found' });
        if (tournament.created_by !== userId)
            return res.status(403).json({ error: 'Only organiser can remove officials' });
        await supabase_1.supabase.from('tournament_officials').delete().eq('id', officialId).eq('tournament_id', id);
...
exports.removeTournamentOfficial = removeTournamentOfficial;
async function getTournamentOfficials(req, res) {
    try {
        const { id } = req.params;
        const { data, error } = await supabase_1.supabase
            .from('tournament_officials')
...
exports.getTournamentOfficials = getTournamentOfficials;
// ────────────────────────────────────────────────────────────────────────────
