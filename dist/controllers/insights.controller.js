...
exports.getUserInsights = exports.getScorerLeaderboard = void 0;
const supabase_1 = require("../utils/supabase");
// ── Scorer Leaderboard ──────────────────────────────────────────────────────
async function getScorerLeaderboard(req, res) {
    try {
...
exports.getScorerLeaderboard = getScorerLeaderboard;
...
async function getUserInsights(req, res) {
    try {
        ...
        const { data: parts } = await supabase_1.supabase
            ...
            .limit(50);
        const completed = (parts ?? []).filter((p) => p.match?.status === 'completed');
        ...
        const formTrend = completed.slice(0, 10).map((p) => {
            const m = p.match;
            const myTeamId = p.team_side === 'A' ? m.team_a_id : m.team_b_id;
        ...
        const { data: ratingHistory } = await supabase_1.supabase
            .from('rating_history')
            .select('new_rating')
            .eq('user_id', id)
            .order('created_at', { ascending: false })
            .limit(10);
        const ratingTrend = (ratingHistory ?? []).map((r) => r.new_rating).reverse();
        // Sport profiles
