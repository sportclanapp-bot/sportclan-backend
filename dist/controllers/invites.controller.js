...
exports.respondToInvite = exports.listInvites = exports.createInvite = void 0;
...
async function createInvite(req, res) {
    ...
    const { data: block } = await supabase_1.supabase
        .from('user_blocks')
        .select('id')
    ...
    const [{ data: sender }, { data: sport }] = await Promise.all([
        supabase_1.supabase.from('users').select('name, username').eq('id', userId).maybeSingle(),
        supabase_1.supabase.from('sports').select('name, emoji').eq('id', sport_id).maybeSingle(),
    ]);
...
async function respondToInvite(req, res) {
    ...
    const { data, error } = await supabase_1.supabase
        ...
        .single();
    if (error || !data)
        return res.status(404).json({ error: error?.message || 'Invite not found' });
