...
async function authorizeScorer(matchId, userId) {
    ...
    if (match.created_by !== userId && match.umpire_id !== userId) {
        return { ok: false, status: 403, error: 'Only the umpire or creator can score' };
    }
...
async function undoEvent(req, res) {
    const userId = req.userId;
    if (!userId)
        return res.status(401).json({ error: 'Unauthorized' });
    try {
        ...
        const { data: latest } = await supabase_1.supabase
            ...
            .maybeSingle();
        if (!latest)
            return res.status(404).json({ error: 'No event to undo' });
        const { error } = await supabase_1.supabase.from('match_events').delete().eq('id', latest.id);
        if (error)
            return res.status(500).json({ error: error.message });
