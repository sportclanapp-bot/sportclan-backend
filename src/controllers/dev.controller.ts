...
const MISSING_SPORTS: Array<{ name: string; slug: string; emoji: string; color: string; display_order: number }> = [
...
const POSTS: Array<{ content: string; type: string }> = [
...
const GIFT_CATALOGUE: Array<{ gift_id: string; gift_emoji: string; gift_name: string; coin_cost: number }> = [
...
function makeScoreSample(sportName: string): Record<string, unknown> {
  switch (sportName) {
    case 'Cricket':
      return {
...
export async function loadTestData(req: Request, res: Response) {
  ...
  try {
    ...
    for (const s of existingSports ?? []) sportsByName.set(s.name.toLowerCase(), s.id);
    ...
    if (missingToInsert.length > 0) {
      const { data: inserted } = await supabase
        ...
        .select('id, name');
      for (const s of inserted ?? []) sportsByName.set(s.name.toLowerCase(), s.id);
    ...
    for (const c of allCities ?? []) citiesByName.set(c.name, c.id);
    const anyCity: string | null = allCities?.[0]?.id ?? null;

    // ── STEP 3: Create 50 test users ───────────────────────────────────
    const NUM_USERS = 50;
    const userRowsToCreate: any[] = [];
    for (let i = 1; i <= NUM_USERS; i++) {
      ...
      if (i > 40 && i <= 45) accountType = 'Umpire';
      else if (i > 45 && i <= 48) accountType = 'Coach';
      else if (i > 48) accountType = 'Business';
    ...
    const { data: existingTeamsRows } = await supabase
      .from('teams')
    ...
    const { data: allTargetTeams } = await supabase
      ...
      .select('id, name, sport_id')
      .in('name', targetTeamNames);
    const teamRecords: Array<{ id: string; name: string; sport_id: string }> = allTargetTeams ?? [];
    ...
    const now = Date.now();
    const day = 86400000;
    const tournamentRowsToInsert: any[] = [];
    ...
    const existingTourneyKeys = new Set(
      (existingTourneys ?? []).map((t) => `${t.name}::${t.sport_id}`),
    );
    const newTourneyRows = tournamentRowsToInsert.filter(
      (t) => !existingTourneyKeys.has(`${t.name}::${t.sport_id}`),
    );
    ...
    const { data: allTargetTourneys } = await supabase
      .from('tournaments')
      .select('id, name, sport_id, status')
      .in('name', targetTournamentNames);
    ...
    const newTourneyIds = new Set(newTourneyRows.map((_, idx) => {
      ...
      const match = tournamentRecords.find(
        (t) => t.name === newTourneyRows[idx].name && t.sport_id === newTourneyRows[idx].sport_id,
      );
      return match?.id;
    }).filter(Boolean));
