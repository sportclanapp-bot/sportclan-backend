...
export async function getCurrentSeason(req: Request, res: Response) {
  ...
  const sportNames = new Map<string, { name: string; emoji: string | null }>();
  ...
  if (medalSeasonIds.length > 0) {
    ...
    for (const s of seasonRows ?? []) seasonNameMap.set(s.id, s.name);
  }
  const medalSportIds = Array.from(new Set((medalRows ?? []).map((m) => m.sport_id).filter(Boolean)));
  const medalSportNameMap = new Map<string, { name: string; emoji: string | null }>();
  if (medalSportIds.length > 0) {
    const { data: sports } = await supabase
      .from('sports')
      .select('id, name, emoji')
      .in('id', medalSportIds);
    for (const s of sports ?? []) medalSportNameMap.set(s.id, { name: s.name, emoji: s.emoji });
  }
...
export async function endSeason(req: Request, res: Response) {
  ...
  if (insertRows.length > 0) {
    await supabase.from('season_medals').insert(insertRows);
  }
