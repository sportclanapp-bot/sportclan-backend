...
export async function getSportProfile(req: Request, res: Response) {
  ...
  try {
    ...
    if (matchIds.length > 0 && ['cricket', 'football', 'basketball'].includes(slug)) {
      ...
      const myEvents = (events ?? []).filter((e: any) => e.created_by === id);
      ...
      if (slug === 'cricket') {
        ...
        for (const e of myEvents) {
          ...
          if (e.event_type === 'ball') {
            ...
            runs += r; balls++; inningsRuns += r;
            if (r === 4) fours++;
            if (r === 6) sixes++;
            if (inningsRuns > hs) hs = inningsRuns;
          }
          if (e.event_type === 'wicket' || pay.wicket) wickets++;
        }
        sportStats = {
        ...
        for (const e of myEvents) {
          if (e.event_type === 'basket' || e.event_type === 'score') points += Number((e.payload as any)?.points ?? 2);
          if (e.event_type === 'foul') fouls++;
        }
